...
class KotlinScriptConfigurationManager(
    ...
    private val cacheManager = ScriptDependenciesUpdater(project, cache, scriptDefinitionProvider)

    init {
        reloadScriptDefinitions()
    }

    fun getScriptClasspath(file: VirtualFile): List<VirtualFile> = toVfsRoots(cacheManager.getCurrentDependencies(file).classpath)
    fun getScriptDependencies(file: VirtualFile) = cacheManager.getCurrentDependencies(file)
    ...
    companion object {
        ...
        fun updateScriptDependenciesSynchronously(virtualFile: VirtualFile, project: Project) {
            with(getInstance(project)) {
                ...
                val updated = cacheManager.updateSync(virtualFile, scriptDefinition)
                assert(updated)
