        ...
        override val uastBody: UExpression? by lz {
            ktClass.getAnonymousInitializers().takeIf { it.isNotEmpty() }?.let {
                object : UBlockExpression {
                    ...
                    override val uastParent: UElement? = this@KotlinConstructorUMethod
                    override val annotations: List<UAnnotation> = emptyList()
                    override val expressions by lz {
                        ktClass.getAnonymousInitializers().map {
...
class KotlinScriptUClass(
    ...
    private fun createUMethod(method: PsiMethod): UMethod {
        return if (method.isConstructor) {
            KotlinScriptConstructorUMethod(psi.files.single().script!!, method as KtLightMethod, this)
        }
        else {
    ...
    class KotlinScriptConstructorUMethod(
        ...
        override val uastBody: UExpression? by lz {
            object : UBlockExpression {
                override val annotations: List<UAnnotation> = emptyList()
                override val psi: PsiElement? = null
                override val uastParent: UElement? = this@KotlinScriptConstructorUMethod
                override val expressions: List<UExpression> = script.declarations.filterIsInstance<KtScriptInitializer>().mapNotNull {
