...
object DeprecatedCallChecker : CallChecker {
    override fun check(resolvedCall: ResolvedCall<*>, reportOn: PsiElement, context: CallCheckerContext) {
        check(resolvedCall.resultingDescriptor, context.trace, reportOn, context.languageVersionSettings, context.deprecationProvider)
    }
    ...
    private fun check(
        ...
        val deprecations = deprecationProvider.getDeprecations(targetDescriptor).toMutableList()
        ...
        if (targetDescriptor is PropertySetterDescriptor) {
            deprecations -= deprecationProvider.getDeprecations(targetDescriptor.correspondingProperty)
        }
        ...
        if (deprecations.isNotEmpty()) {
            ...
            targetDescriptor.getter?.let { check(it, trace, element, languageVersionSettings, deprecationProvider) }
        }
    }
