...
class IrExpressionTranslationVisitor(private val context: IrTranslationContext) : IrElementVisitor<JsExpression?, Unit> {
    ...
    override fun visitCall(expression: IrCall, data: Unit): JsExpression? {
        ...
        return when (function) {
            is PropertyGetterDescriptor -> {
                when {
                    JsDescriptorUtils.shouldAccessViaFunctions(function) -> {
                        JsInvocation(context.translateAsValueReference(function), arguments)
                    }
            ...
            is PropertySetterDescriptor -> {
                val statement = when {
                    JsDescriptorUtils.shouldAccessViaFunctions(function) -> {
                        buildJs { context.translateAsValueReference(function).invoke(*arguments.toTypedArray()) }
                    }
                    dispatchReceiver != null -> {
                        buildJs {
    ...
    override fun visitTypeOperator(expression: IrTypeOperatorCall, data: Unit): JsExpression? = expression.argument.accept(this, Unit)

    override fun visitBlock(expression: IrBlock, data: Unit): JsExpression? {
        val jsBlock = JsBlock()
